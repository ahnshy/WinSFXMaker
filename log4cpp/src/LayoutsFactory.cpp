...
namespace log4cpp
{
   ...
   LayoutsFactory& LayoutsFactory::getInstance()
   {
      if (!layouts_factory_)
      {
         std::LOG4CPP_UNIQUE_PTR<LayoutsFactory> lf(new LayoutsFactory);
   ...
   std::LOG4CPP_UNIQUE_PTR<Layout> LayoutsFactory::create(const std::string& class_name, const params_t& params)
   {
      const_iterator i = creators_.find(class_name);
      if (i == creators_.end())
